...
FE::EditorViewWindow::EditorViewWindow(const std::string& aHandle, bool aOpen, ImGuiWindowFlags aFlags)
	: WindowBase(aHandle, aOpen, aFlags)
{}

void FE::EditorViewWindow::Show(const EditorUpdateContext& aContext)
{
	aContext;
	...
	if (ImGui::Begin(myData.handle.c_str(), &myData.isOpen, myData.flags))
	{
		...
		tDrawList->AddImage(0, vMinWin, vMaxWin);
	}
	ImGui::End();
}
