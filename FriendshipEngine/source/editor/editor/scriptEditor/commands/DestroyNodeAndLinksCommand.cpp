...
#include <game/scripting/nodes/ScriptNodeBase.h>
...
void DestroyNodeAndLinksCommand::ExecuteImpl()
{
	for (std::pair<const ScriptNodeId, CommandNodeData>& node : myNodes)
	{
		...
		//InputPins
		{
			...
			for (size_t pinIndex = 0; pinIndex < inputPinCount; pinIndex++)
			{
...

		{
			...
			const ScriptPinId* pins = myScript.GetOutputPins(nodeId, outputPinCount);

			for (size_t pinIndex = 0; pinIndex < outputPinCount; pinIndex++)
			{
				ScriptPinId pin = pins[pinIndex];
	...
	for (std::pair<const ScriptNodeId, CommandNodeData>& node : myNodes)
	{
		...
		nodeData.typeId = myScript.GetNodeType(nodeId);
		nodeData.pos = myScript.GetNodePosition(nodeId);
		nodeData.instanceName = myScript.GetNodeName(nodeId);
