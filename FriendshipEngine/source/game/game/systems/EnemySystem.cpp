...
void EnemySystem::Update(SceneUpdateContext& aContext)
{
	...
	{
		playerComp = &myWorld->GetComponent<PlayerComponent>(playerID);
	}
	...
	for (Entity entity : myEntities)
	{
		...
		auto& controller = AssetDatabase::GetAnimationController(anim.controllerId);
	...
	if (DmgPlayer && playerID != INVALID_ENTITY)
	{
		playerComp->isChased = true;
	}
	else if (playerID != INVALID_ENTITY)
	{
		playerComp->isChased = false;
	}
