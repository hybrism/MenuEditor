...
public:
	...
	void WriteToJson(ScriptJson& result);

	ScriptNodeId CreateNode(ScriptNodeTypeId typeId);
	ScriptLinkId CreateLink();
	ScriptPinId CreatePin();

	ScriptNodeId CreateNode(ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& node, Vector2f nodePos);
	...
	/// </summary>
	void CreateNodeWithReusedId(ScriptNodeId id, ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& node, Vector2f nodePos);
	/// <summary>
	...
	ScriptNodeTypeId GetNodeType(ScriptNodeId id) const;
	const ScriptNodeBase& GetNode(ScriptNodeId id) const;
	ScriptNodeBase& GetNode(ScriptNodeId id);
	ScriptStringId GetNodeName(ScriptNodeId id) const;
	Vector2f GetNodePosition(ScriptNodeId id) const;
	...
	void SetPosition(ScriptNodeId id, Vector2f nodePos);
