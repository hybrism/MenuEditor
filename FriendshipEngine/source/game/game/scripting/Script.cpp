...
#include "nodes/ScriptNodeTypeRegistry.h"
...
void Script::LoadFromJson(const ScriptJson& data)
{
	...
	for (json::const_iterator it = nodes.begin(); it != nodes.end(); ++it)
	{
		...
		if (!instanceName.empty())
			SetName(id, ScriptStringRegistry::RegisterOrGetStringId(instanceName));
	...
	for (json::const_iterator it = pins.begin(); it != pins.end(); ++it)
	{
		...
		CreatePinWithReusedId(id, { role, INT_MAX, nodeId, ScriptStringRegistry::RegisterOrGetStringId(name), dataType, std::monostate(), overridenValue });
	}
...
void Script::WriteToJson(ScriptJson& result)
{
	...
	for (ScriptNodeId nodeId = GetFirstNodeId(); nodeId.id != ScriptNodeId::InvalidId; nodeId = GetNextNodeId(nodeId))
	{
		...
		ScriptNodeTypeId type = GetNodeType(nodeId);
		ScriptStringId name = GetNodeName(nodeId);
		Vector2f pos = GetNodePosition(nodeId);
...
ScriptNodeId Script::CreateNode(ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& newNode, Vector2f nodePos)
{
	UpdateSequenceNumber();
...
void Script::CreateNodeWithReusedId(ScriptNodeId id, ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& newNode, Vector2f nodePos)
{
	UpdateSequenceNumber();
...
ScriptNodeTypeId Script::GetNodeType(ScriptNodeId id) const
{
	assert(id.id < myNodes.size());
...
ScriptStringId Script::GetNodeName(ScriptNodeId id) const
{
	assert(id.id < myNodes.size());
	assert(myNodes[id.id].isRemoved == false);
...
Vector2f Script::GetNodePosition(ScriptNodeId id) const
{
	assert(id.id < myNodes.size());
	assert(myNodes[id.id].isRemoved == false);

	return myNodes[id.id].nodePosition;
}
...
void Script::SetPosition(ScriptNodeId id, Vector2f nodePos)
{
	UpdateSequenceNumber();
	...
	myNodes[id.id].nodePosition = nodePos;
}
