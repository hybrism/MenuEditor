...
ScriptNodeTypeId ScriptNodeTypeRegistry::GetTypeId(std::string_view typeName)
{
...
std::unique_ptr<ScriptNodeBase> ScriptNodeTypeRegistry::CreateNode(ScriptNodeTypeId typeId)
{
...
std::string_view ScriptNodeTypeRegistry::GetNodeTypeShortName(ScriptNodeTypeId typeId)
{
	assert("Invalid type id" && typeId.id < myTypeInfos.size());
...
std::string_view ScriptNodeTypeRegistry::GetNodeTypeFullName(ScriptNodeTypeId typeId)
{
	assert("Invalid type id" && typeId.id < myTypeInfos.size());
...
std::string_view ScriptNodeTypeRegistry::GetNodeTooltip(ScriptNodeTypeId typeId)
{
	assert("Invalid type id" && typeId.id < myTypeInfos.size());
...
ScriptNodeTypeRegistry::TypeInfo& ScriptNodeTypeRegistry::RegisterTypeInternal(const char* fullName, const char* toolTip)
{
	...
	{
		ScriptStringId stringId = ScriptStringRegistry::RegisterOrGetStringId(s.c_str());
