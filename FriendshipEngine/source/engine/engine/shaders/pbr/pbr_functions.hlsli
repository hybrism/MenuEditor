...
float3 EvaluateAmbiance(TextureCube lysBurleyCube, float3 vN, float3 VNUnit, float3 toEye, float perceptualRoughness, float ao, float3 dfcol, float3 spccol)
{
    ...
    float3 specRad = lysBurleyCube.SampleLevel(aDefaultSampler, vR, mipLevel).xyz;
    float3 diffRad = lysBurleyCube.SampleLevel(aDefaultSampler, vN, (float) (nrBrdMips - 1)).xyz;
