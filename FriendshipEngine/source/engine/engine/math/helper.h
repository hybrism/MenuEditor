...
#pragma warning(disable:4146)
	...
	static float GetNoise(float amount)
	{
		return (2.0f * RandomFloat() - 1.0f) * amount;
	}

	static void AddNoise(std::vector<float>& buffer, float amount)
	{
		for (int i = 0; i < buffer.size(); i++)
		{
			buffer[i] += GetNoise(amount);
		}
	}
