...
void GlyphLoader::CalculateOutlineOffsets(const int index, FT_FaceRec_* aFace, int aBorderOffset)
{
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to load glyph!");
	}
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to get glyph!");
	}
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to stroke glyph border");
	}
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to add glyph to bitmap");
	}
...
void GlyphLoader::LoadGlyph(int index, int& atlasX, int& atlasY, int& maxY, float atlasWidth, float atlasHeight, InternalTextAndFontData* aFontData, FT_FaceRec_* aFace, int aBorderOffset)
{
	...
	if (error != 0)
	{
		PrintE("[TextService.cpp] Failed to load char while glyph loading");
		return;
	}
	...
	{
		PrintE("[TextService.cpp] Tried to set a glyph UV to above 1");
		return;
	}
...
void GlyphLoader::LoadOutline(const int index, const int atlasX, const int atlasY, const float atlasWidth, InternalTextAndFontData* aFontData, FT_FaceRec_* aFace, int aBorderOffset)
{
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to get stroker!");
	}
	...
	if (err != 0)
	{
		PrintE("[TextService.cpp] Failed to stroke!");
	}
...
TextToRender processNextCharacter(const InternalTextAndFontData& fontData, char aCharacter, float aSize, float& x, float& y, float& maxY)
{
	...
	float pos_x = static_cast<float>(((x + charData.myBearingX) * aSize));
	float pos_y = static_cast<float>(((y + charData.myBearingY) * aSize));

	result.myPosition = { pos_x, pos_y };
	result.mySize = { static_cast<float>(charData.myWidth), static_cast<float>(charData.myHeight) };
...
Font TextService::GetOrLoad(std::wstring aFontFile, unsigned char aBorderSize, int aSize)
{
	if (aSize == -1)
	{
		return { nullptr };
	...
	if (error != 0)
	{
		PrintE("[TextService.cpp] FT_Set_Char_Size error");
		return { nullptr };
	}
...
bool TextService::Draw(Text& aText, SpriteShader* aCustomShader)
{
	...
	if (rotation != 0.0f)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			minX = std::min(minX, charInfo.myPosition.x);
			minY = std::min(minY, charInfo.myPosition.y);
			maxX = std::max(maxX, charInfo.myPosition.x + charInfo.mySize.x);
			maxY = std::max(maxY, charInfo.myPosition.y + charInfo.mySize.y);
		}
...

	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (rotation != 0.0f)
			{
				spriteInstance.rotation = rotation;
